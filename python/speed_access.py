                ...
                if travel_time > 0:
                    ...
                    if length is not None:
                        ...
                        results.append({
                            ...
                            'speed': speed,
                            'freespeed': freespeed,
                            'freespeed_distance': link_statistics[link]['freespeed_distance'],
                            'stop_and_go_distance': link_statistics[link]['stop_and_go_distance'],
                            'freespeed_time': link_statistics[link]['freespeed_time'],
                            'stop_and_go_time': link_statistics[link]['stop_and_go_time']
                        })
                    else:
...
def plot_speeds(results, vehicle_id, start_time=None, end_time=None):
    ...
    filtered_data = [
        r for r in vehicle_data
        if (start_time is None or r['entry_time'] >= start_time) and
           (end_time is None or r['exit_time'] <= end_time)
    ]
    ...
    # Calculate speed for each time interval, keeping the speed constant until the status changes
    times = []
    speeds = []
    current_speed = None
    current_time = None
